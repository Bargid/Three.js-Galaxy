...
const generateGalaxy = () => {
    ...
    // Destroy Old Galaxy
    if (points !== null) { // on se debarasse des anciennes geometries et materiaux lorsqu'on ajuste le GUI
    ...
    const positions = new Float32Array(parameters.count * 3) // 3 pour XYZ
    const colors = new Float32Array(parameters.count * 3) // 3 pour RGB
    ...
    for (let i = 0; i < parameters.count; i++) {
        ...
        // Color
        const mixedColor = colorInside.clone() // on clone pour ne pas modifier la couleur initiale
...
const tick = () =>
{
    ...
    // Geometry Rotation
    if (points) {
        const rotationSpeed = parameters.rotationSpeed
...
gui.add(parameters, 'rotationSpeed').min(0).max(1).step(0.01).name('Rotation Speed').onFinishChange(generateGalaxy)
